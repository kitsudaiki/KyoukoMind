 ...
 */
bool
Brick::initEdgeSectionBlocks(const uint32_t numberOfEdgeSections)
{
    if(edges.inUse != 0)
    {
        // TODO: log-output
    ...
    EdgeSection* array = getEdgeBlock(*this);
...
/**
 * @brief addEmptyEdgeSection
 * @param sourceSide
 * @param sourceId
 * @return
 */
uint64_t
Brick::addEmptyEdgeSection(const uint8_t sourceSide,
                           const uint32_t sourceId)
{
    ...
    const uint64_t position = reserveDynamicItem(edges);
    ...
    getEdgeBlock(*this)[position] = newSection;
    assert(newSection.sourceSide != 0);
    assert(getEdgeBlock(*this)[position].sourceSide != 0);
